...
<html lang="en">
  ...
  <body>  
    ...
    <br><br><br><br><br><br>
      <center><h2>Asignar Evaluador</h2></center>
    <br>
      <form method="post">
        <center><table class="tabla">
              <c:forEach var="itema" items="${universidad.rows}">
                ...
                <tr><td>Evaluador</td>
                    <td>
                        <sql:query var="rsEvaluador" dataSource="${parcialsegundocorte}">
                        SELECT * FROM evaluador
                    </sql:query>

                        <select name="evaluador_est">
                            <option value="0">--Seleccione--</option>
                            <c:forEach var="evaluador" items="${rsEvaluador.rows}">
                                <option value="${evaluador.id}">
                                    <c:if test="${evaluador.id == itema.evaluador_est}">
                                    </c:if>              
                                <c:out value="${evaluador.nom_evaluador}"/>
                                </option>
                            </c:forEach>
                        </select>
                    </td>
    ...
    <c:if test="${param.modifica != null}" >
      <sql:update var="result" dataSource="${parcialsegundocorte}">
          ...
          evaluador_est = '${param.evaluador_est}',
      ...
      <br><br><br><br> <br><br><br><br>
        <center>  <div class="card" style="width: 18rem;">
          <div class="card-body">
            <p class="text-black">Evaluador Asignado Correctamente!</p>
            <a class="btn btn-dark" href="vista_anteproyectoeva.jsp">Regresar</a>
          </div>
        </div></center>
      </c:if>
    </c:if>
